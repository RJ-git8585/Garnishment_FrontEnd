...
const ExemptAmountPopup = ({ open, handleClose, state }) => {
  ...
  useEffect(() => {
    const fetchExemptData = async () => {
      ...
      setLoading(true);
      try {
        const response = await fetch(`${API_URLS.GET_GARNISHMENT_EXEMPT_CONFIG}${state.toLowerCase()}/`);
        if (!response.ok) {
    ...
    <Dialog open={open} onClose={handleClose} maxWidth="lg" fullWidth>
      <DialogTitle className="bg-gray-100">
        <div className="flex justify-between items-center">
          <span className='capitalize'>Exempt Amount Configuration - {state}</span>
...
export default ExemptAmountPopup; 